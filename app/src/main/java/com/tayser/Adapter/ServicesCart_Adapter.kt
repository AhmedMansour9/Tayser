...
class ServicesCart_Adapter (context: Context, val userList: List<CartService_Response.Data.Details>)
    : RecyclerView.Adapter<ServicesCart_Adapter.ViewHolder>() {
...
//    lateinit var checkbox: ChechBox_View
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): ServicesCart_Adapter.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.row_servicecart, parent, false)
        return ViewHolder(v)
    }
    override fun onBindViewHolder(holder: ServicesCart_Adapter.ViewHolder, position: Int) {
        holder.itemView.T_Title.text=userList.get(position).name
        holder.itemView.T_Price.text=userList.get(position).totalServiceQuantity+" "+ context.getString(R.string.currency)
